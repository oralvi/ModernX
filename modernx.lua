...
local user_opts = {
    ...
    windowcontrols = 'auto',        -- 是否显示 OSC 窗口控件, 'auto', 'yes' or 'no'
    showwindowed = true,            -- 窗口化时显示 OSC？
    showfullscreen = true,          -- 全屏时显示 OSC？
    noxmas = false,                 -- disable santa hat in December
    
    -- scaling settings --
    vidscale = true,                -- 是否随视频缩放控制器
    scalewindowed = 1.1,            -- 窗口化时控制器的缩放
    scalefullscreen = 0.7,          -- 全屏时控制器的缩放
    scaleforcedwindow = 1.0,        -- 在强制窗口上渲染时缩放

    -- interface settings --
    hidetimeout = 1500,             -- 无操作时 OSC 的持续时间（以毫秒为单位）
    fadeduration = 150,             -- 淡出持续时间（以毫秒为单位）
    minmousemove = 0,               -- 显示 osc 时鼠标必须移动的像素量
    scrollingSpeed = 40,            -- 菜单中滚动文本的速度
    showonpause = false,            -- 是否禁用暂停时隐藏超时
    donttimeoutonpause = false,     -- 是否要在暂停时禁用隐藏超时
    bottomhover = true,             -- osc 是否仅在鼠标悬停在底部时显示
    raisesubswithosc = true,        -- 显示时是否将字幕升到屏幕上方
    thumbnailborder = 0,            -- 缩略图边框的宽度
    persistentprogress = false,     -- 总是在屏幕底部显示一条小进度条
    persistentprogressheight = 18,  -- persistentprogress 进度条的高度
    persistentbuffer = false,       -- 在网络视频上，显示在 persistentprogress 进度条上的缓冲进度条

    -- title and chapter settings --
    showtitle = true,		        -- 在 OSC 中显示标题
    showdescription = true,         -- 在网络视频上显示视频描述
    showwindowtitle = false,        -- 以无边框/全屏模式显示窗口标题
    titleBarStrip = false,          -- 是否将标题栏设置为单一条而不是黑色渐变效果 
    title = '${media-title}',       -- 在 OSC 上显示的标题 - 关闭 dynamictitle 以应用此选项
    dynamictitle = true,            -- 根据 {media-title} 和 {filename} 来更改标题
                                    --（例如播放url、音频或某些媒体）
    updatetitleyoutubestats = false,-- 使用YouTube视频统计数据更新窗口/OSC标题栏（浏览次数、喜欢和不喜欢）
    font = 'mpv-osd-symbols',       -- 默认OSC字体
                                    -- 以OSC标题的形式显示
    titlefontsize = 26,             -- 标题文本的字体大小
    chapterformat = 'Chapter: %s',  -- seekbar-hover的章节打印格式。“no”表示禁用
    dateformat = "%Y-%m-%d",        -- 元数据读取时日期应如何格式化
                                    --（使用标准lua日期格式）
    osc_color = '000000',           -- OSC 和标题栏的强调色
    OSCfadealpha = 100,             -- OSC 背景框的透明度
    boxalpha = 75,                  -- 窗口标题栏的透明度
    descriptionBoxAlpha = 100,      -- 描述背景框的透明度

    -- seekbar settings --
    seekbarfg_color = 'E39C42',     -- 进度条和控制按钮的颜色
    seekbarbg_color = 'FFFFFF',     -- 剩余进度条的颜色
    seekbarkeyframes = false,       -- 拖动进度条时是否使用关键帧
    seekbarhandlesize = 0.5,        -- 滑块手柄的大小比例，范围在 0 到 1 之间
    seekrange = true,               -- 显示搜索范围叠加
    seekrangealpha = 150,           -- 搜索范围叠加的透明度
    iconstyle = 'round',            -- 图标样式，'solid' 或 'round'
    hovereffect = true,             -- 当鼠标悬停在按钮上时是否有发光效果

    -- button settings --
    timetotal = true,               -- 默认显示总时间而不是剩余时间
    timems = false,                 -- 默认不以毫秒显示时间
    timefontsize = 17,              -- 时间的字体大小
    jumpamount = 5,                 -- 更改跳转量（默认为秒）
    jumpiconnumber = true,          -- 当跳转量为 5、10 或 30 时显示不同的图标
    jumpmode = 'exact',             -- 跳转按钮的寻找模式，例如 'exact'、'relative+keyframes' 等
    volumecontrol = true,           -- 是否显示静音按钮和音量滑块
    volumecontroltype = 'linear',   -- 使用线性或对数音量刻度
    showjump = false,               -- 显示“向前/向后跳转 5 秒”按钮
    showskip = true,                -- 显示快退和快进（章节）按钮
    compactmode = false,            -- replace the jump buttons with the chapter buttons, clicking the
                                    ...
                                    -- skipping a chapter
    showloop = false,               -- 显示循环（loop）按钮
    loopinpause = true,             -- 右键单击暂停激活循环
    showontop = true,               -- 显示窗口置顶按钮
    showinfo = true,                -- 显示信息按钮
    downloadbutton = true,          -- 为网络视频显示下载按钮
    downloadpath = "~~desktop/mpv/downloads", -- 视频下载路径
    ytdlpQuality = '-f bestvideo[vcodec^=avc][ext=mp4]+bestaudio[ext=m4a]/best[ext=mp4]/best' -- 下载按钮使用的视频质量（默认为最高质量 mp4）
}
...
local iconfont = user_opts.iconstyle == 'round' and 'Material-Design-Iconic-Round' or 'Material-Design-Iconic-Font'
    -- 按钮元素样式：blur颜色渐变 bord颜色宽度 1c&色值  fs大小
local osc_styles = {
    TransBg = "{\\blur20\\bord40\\" .. user_opts.OSCfadealpha .. "\\1c&H000000&\\3c&H" .. user_opts.osc_color .. "&}",
    ...
    Ctrl1 = '{\\blur0\\bord0\\1c&HFFFFFF&\\3c&HFFFFFF&\\fs35\\fn' .. iconfont .. '}',
    Ctrl2 = '{\\blur0\\bord0\\1c&HFFFFFF&\\3c&HFFFFFF&\\fs25\\fn' .. iconfont .. '}',
    Ctrl2Flip = '{\\blur0\\bord0\\1c&HFFFFFF&\\3c&HFFFFFF&\\fs25\\fn' .. iconfont .. '\\fry180',
    Ctrl3 = '{\\blur0\\bord0\\1c&HFFFFFF&\\3c&HFFFFFF&\\fs25\\fn' .. iconfont .. '}',
    Time = '{\\bord1\\bord3\\1c&HFFFFFF&\\3c&H000000&\\fs' .. user_opts.timefontsize .. '\\fn' .. user_opts.font .. '}',
    Tooltip = '{\\blur1\\bord2\\1c&HFFFFFF&\\3c&H000000&\\fs' .. user_opts.timefontsize .. '\\fn' .. user_opts.font .. '}',
    Title = '{\\bord1\\bord2\\1c&HFFFFFF&\\3c&H0\\fs'.. user_opts.titlefontsize ..'\\q2\\fn' .. user_opts.font .. '}',
    WindowTitle = '{\\blur1\\bord0.5\\1c&HFFFFFF&\\3c&H0\\fs'.. 20 ..'\\q2\\fn' .. user_opts.font .. '}',
    Description = '{\\blur1\\bord0.5\\1c&HFFFFFF&\\3c&H000000&\\fs'.. 50 ..'\\q2\\fn' .. user_opts.font .. '}',
...
function normaliseDate(date)
    ...
    elseif (#date > 6) then -- YYYYMMDD
        local dateTable = {year = date:sub(1,4), month = date:sub(5,6), day = date:sub(7,8)}
        return os.date(user_opts.dateformat, os.time(dateTable))
    elseif (#date >4) then --YYYYMM
        local dateTable = {year = date:sub(1,4), month = date:sub(5,6),day = 01}
        return os.date(user_opts.dateformat, os.time(dateTable))
    else -- YYYY
        return date
    end
...
function window_controls()
    ...
    ne.eventresponder['mbtn_left_up'] =
        function () mp.commandv('quit-watch-later') end
    lo = add_layout('close')
...
layouts = function ()
    ...
    -- Seekbar 进度条： x=左右  y=上下   w=进度条与左右边界的距离   h=进度条宽度
    new_element('seekbarbg', 'box')
    lo = add_layout('seekbarbg')
    lo.geometry = {x = refX -0, y = refY - 45 , an = 5, w = osc_geo.w - 30, h = 3}
    lo.layer = 13
    ...
    lo.geometry = {x = refX -0, y = refY - 45 , an = 5, w = osc_geo.w - 30, h = 17}
    ...
    -- Volumebar  音量条： x=左右  y=上下  w=音量条长度 h=高度
    lo = new_element('volumebarbg', 'box')
    lo.visible = (osc_param.playresx >= 900 - outeroffset) and user_opts.volumecontrol
    lo = add_layout('volumebarbg')
    lo.geometry = {x = osc_geo.w - 300, y = refY - 20, an = 4, w = 80, h = 2}
    lo.layer = 13
    lo.alpha[1] = 128
    lo.style = osc_styles.VolumebarBg
    
    lo = add_layout('volumebar')
    lo.geometry = {x = osc_geo.w - 300, y = refY - 20, an = 4, w = 80, h = 8}
    lo.style = osc_styles.VolumebarFg
    lo.slider.gap = 3
    lo.slider.tooltip_style = osc_styles.Tooltip
    lo.slider.tooltip_an = 2

	-- buttons  播放/上下一章节/步进步退五秒/前后文件/控制按钮的位置
    lo = add_layout('pl_prev')
    if showskip then
        lo.geometry = {x = 140, y = refY - 25 , an = 5, w = 30, h = 24}
    else
        lo.geometry = {x = 70 - offset, y = refY - 25 , an = 5, w = 30, h = 24}
    end
    lo.style = osc_styles.Ctrl2

    if showskip then 
        lo = add_layout('skipback')
        lo.geometry = {x = 170 , y = refY - 25 , an = 5, w = 30, h = 24}
        lo.style = osc_styles.Ctrl2
    end
    ...
    lo.geometry = {x = 210 , y = refY - 25 , an = 5, w = 45, h = 45}
    ...
    if showskip then
        ...
        lo.geometry = {x = 250 , y = refY - 25 , an = 5, w = 30, h = 24}
        lo.style = osc_styles.Ctrl2	
    end

    lo = add_layout('pl_next')
    if showskip then
        lo.geometry = {x = 280 , y = refY - 25 , an = 5, w = 30, h = 24}
    else
        lo.geometry = {x = 140 , y = refY - 25 , an = 5, w = 30, h = 24}
    end
    lo.style = osc_styles.Ctrl2

	-- Time  osc时间： x=左右  y=上下
    lo = add_layout('tc_left')
    lo.geometry = {x = osc_geo.w - 110, y = refY - 37, an = 9, w = 64, h = 20}
    lo.style = osc_styles.Time	
	
    lo = add_layout('tc_right')
    lo.geometry = {x = osc_geo.w - 110 , y = refY -20, an = 9, w = 64, h = 20}
    lo.style = osc_styles.Time	

    -- Audio/Subtitle cy_audio：音频轨道  cy_sub：字幕轨道  vol_ctrl：音量图标 
    lo = add_layout('cy_audio')
	lo.geometry = {x = 60, y = refY - 25, an = 5, w = 24, h = 24}
    lo.style = osc_styles.Ctrl3
    lo.visible = (osc_param.playresx >= 500 - outeroffset)
	
    lo = add_layout('cy_sub')
    lo.geometry = {x = 100, y = refY - 25, an = 5, w = 24, h = 24}
    lo.style = osc_styles.Ctrl3
    lo.visible = (osc_param.playresx >= 600 - outeroffset)

    lo = add_layout('vol_ctrl')
    lo.geometry = {x = osc_geo.w - 320 , y = refY - 20, an = 5, w = 24, h = 24}
    lo.style = osc_styles.Ctrl3
    lo.visible = (osc_param.playresx >= 700 - outeroffset)

    -- Fullscreen/Loop/Info  tog_fs：全屏图标 tog_ontop：置顶图标
    ...
    if showontop then
        ...
        lo.geometry = {x = osc_geo.w - 127 + (showloop and 0 or 50), y = refY - 20, an = 5, w = 24, h = 24}
        lo.style = osc_styles.Ctrl3
        lo.visible = (osc_param.playresx >= 700 - outeroffset)
    end
    ...
    if showinfo then
        ...
        lo.geometry = {x =  20 , y = refY - 25, an = 5, w = 24, h = 24}
        lo.style = osc_styles.Ctrl3
        lo.visible = (osc_param.playresx >= 500 - outeroffset)
    end
...
function adjustSubtitles(visible)
    if visible and user_opts.raisesubswithosc and state.osc_visible == true and (state.fullscreen == false or user_opts.showfullscreen) then
        local w, h = mp.get_osd_size()
        if h > 0 then
            mp.commandv('set', 'sub-pos', math.floor((osc_param.playresy - 50)/osc_param.playresy*100)) -- percentage
        end
